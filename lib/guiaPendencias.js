...
export function checklistPendencias({ enquadramento, situacao }) {
  ...
  return [
    {
      titulo: "1. Antes de tudo: descubra a data de ciência — não assuma que o prazo ainda está aberto",
      texto:
        "No e-CAC (Caixa Postal/DTE), abra o Termo de Exclusão e veja a data de ciência que o próprio sistema mostra — é esse número que conta, não a data de hoje em que você está olhando. Se a caixa postal nunca tinha sido acessada, a ciência pode já ter acontecido sozinha: no DTE do Simples Nacional (DTE-SN), a intimação é considerada recebida automaticamente em até 45 dias corridos após ser disponibilizada, mesmo sem ninguém abrir a mensagem. Só depois de confirmar essa data dá para saber se os 30 dias para reverter a exclusão quitando o Relatório de Pendências ainda estão correndo ou já passaram.",
